...
const OldaHockeyMatch = ({ onBack, onGameComplete, assignedJerseys, playerName = 'Nový hráč', playerLevel = 1 }) => {
  ...
  useEffect(() => {
      ...
      const toiInterval = setInterval(() => {
          setPlayerStats(prevStats => {
              ...
              // Použijeme aktuální teamState pro zjištění, kdo je na ledě
              let currentTeamState;
              setTeamState(ts => { currentTeamState = ts; return ts; });
              if (!currentTeamState) return prevStats; // Pokud stav není k dispozici

              ['white', 'black'].forEach(teamColor => {
                  const playersOnIceKeys = new Set(currentTeamState[teamColor]?.onIce?.map(p => p?.key).filter(Boolean) || []);
  ...
  }, [gameState, gameSpeed, setTeamState]); // Spustí se znovu při změně stavu hry nebo rychlosti
  ...
  const isPlayerOnIce = useCallback((teamColor) => {
      ...
      // Potřebujeme aktuální stav
      let currentTeamState;
      setTeamState(ts => { currentTeamState = ts; return ts; });
      if (!currentTeamState || !currentTeamState[teamColor]?.onIce) return false;
      return currentTeamState[teamColor].onIce.some(p => p && p.isPlayer);
  }, [setTeamState]); // Závislost na setTeamState pro získání aktuálního stavu

  // --- generateSpecialAction ---
  const generateSpecialAction = useCallback((playerTeamColor, currentTime) => {
    let currentTeamState;
    setTeamState(ts => { currentTeamState = ts; return ts; });
    if (!currentTeamState) return; // Pokud stav není k dispozici
  ...
  }, [setTeamState, score, currentPeriod, GAME_DURATION_SECONDS]); // Závislost na score, currentPeriod
